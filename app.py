...
def main():
    ...
    st.title("🧾 Générateur PDF / Word (Excel désactivé)")
    st.success("✅ Interface chargée")

    try:
        uploaded_file = st.file_uploader("📄 Fichier Excel", type=["xlsx"])
        format = st.selectbox("📂 Format de sortie", ["PDF", "Word", "Excel (non actif)"])
        ...
        margin_left = st.number_input("⬅️ Marge gauche (mm)", value=10.0)

        submitted = st.button("Générer")

        if submitted and uploaded_file:
            if format == "Excel (non actif)":
                st.warning("⚠️ Génération Excel temporairement désactivée pour investigation.")
                return
            ...
            if format == "Word":
                ...
                x0 = mm_to_pt(margin_left)
                y0 = page_height - mm_to_pt(margin_top)
                dx = mm_to_pt(label_w + spacing_x)
                dy = mm_to_pt(label_h + spacing_y)
                ...
                for i, code in enumerate(codes):
                    ...
                    x = x0 + col * dx
                    y = y0 - row * dy
                    c.drawImage(tmp, x, y - mm_to_pt(label_h), width=mm_to_pt(label_w), height=mm_to_pt(label_h))
